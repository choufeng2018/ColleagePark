...
public class ParkServiceBannerView implements Holder<List<MainMenuTo>> {
    ...
    public void UpdateUI(Context context, int position, List<MainMenuTo> data) {
        for (int i = 0; i < 8&&i<data.size(); i++) {
            View mView = View.inflate(context, R.layout.park_service_item, null);
            mView.setTag(data.get(i).getCode() + "");
            ImageView propertyImage = (ImageView) mView.findViewById(R.id.park_service_image);
            TextView propertyName = (TextView) mView.findViewById(R.id.park_service_name);
